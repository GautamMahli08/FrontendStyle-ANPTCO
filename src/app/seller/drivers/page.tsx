...
export default function SellerDriversPage() {
  ...
  useEffect(() => {
    if (!user) return;
    const iv = setInterval(() => loadData(user), 3000);
    return () => clearInterval(iv);
  }, [user]);

  const loadData = (currentUser: any) => {
    setDrivers(getDrivers().filter((d: any) => d.workspaceId === currentUser.workspaceId));
  ...
  const handleAddDriver = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const newDriver = {
        ...
        licenseNumber,
        tspId: user.id,
        tspName: user.companyName || user.firstName,
      ...
      if (assignedTruckId) {
        updateTruck(assignedTruckId, { assignedDriverId: newDriver.id });
      }
  ...
  const availableTrucks = trucks.filter(truck => !truck.assignedDriverId);
        ...
        <main className="p-8">
          <div className="mb-8 flex items-center justify-between">
            <div>
              ...
              <p className="text-gray-600">Manage your driver team — {user.companyName ?? 'this tenant'}</p>
            </div>
            <button
          ...
          {drivers.length > 0 ? (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {drivers.map((driver) => {
                const truck = trucks.find(t => t.assignedDriverId === driver.id);
                return (
                  <div key={driver.id} className={`rounded-xl p-6 border ${
                    ...
                    <div className="flex items-start justify-between mb-4">
                      ...
                      {driver.disabled ? (
                        <span className="px-3 py-1 rounded-full text-xs font-medium bg-red-100 text-red-800">🚫 Disabled</span>
                      ) : (
                    ...
                    <div className="pt-3 border-t flex items-center justify-between">
                      <span className="text-xs font-medium text-gray-500">{driver.disabled ? 'Driver disabled' : 'Driver active'}</span>
                      <ToggleSwitch size="sm" enabled={!driver.disabled} onChange={() => toggleDriver(driver)} />
                    </div>
                  </div>
                );
              })}
            ...
            <form onSubmit={handleAddDriver} className="space-y-4">
              ...
              <div>
                ...
                <input type="tel" value={phone} onChange={(e) => setPhone(e.target.value)}
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                  placeholder="+968 XXXX XXXX" required />
              </div>
              ...
              <div>
                ...
                <input type="text" value={licenseNumber} onChange={(e) => setLicenseNumber(e.target.value)}
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                  placeholder="DL-XX-XXXXXX" required />
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Assign Truck (Optional)</label>
                <select value={assignedTruckId} onChange={(e) => setAssignedTruckId(e.target.value)}
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
                  <option value="">-- Select truck --</option>
                  {availableTrucks.map(truck => (
                    <option key={truck.id} value={truck.id}>{truck.registrationNumber} — {truck.status}</option>
                  ))}
                </select>
                <p className="text-xs text-gray-500 mt-1">
                  {availableTrucks.length === 0 ? 'No available trucks.' : 'Assign a truck to this driver'}
                </p>
              </div>
