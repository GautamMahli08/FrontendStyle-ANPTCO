...
import { api, type ApiWorkspaceModules } from '@/src/lib/api';

type NavItem = { name: string; path: string; icon: string };

const baseNav: Record<UserRole, NavItem[]> = {
  PLATFORM_ADMIN: [
    { name: 'Dashboard',          path: '/platform-admin/dashboard',          icon: '🏠' },
    { name: 'Onboarding',         path: '/platform-admin/onboarding',         icon: '🚀' },
    { name: 'Device Inventory',   path: '/platform-admin/devices',            icon: '📡' },
  ...
  TRANSPORT_ADMIN: [
    ...
    // Ordering-module items — hidden when modules.ordering = false
  ...
  ERP_CLIENT: [
    { name: 'Dashboard',    path: '/erp-client/dashboard',    icon: '🏠' },
    { name: 'Live Status',  path: '/erp-client/live-status',  icon: '🗺️' },
    { name: 'Events',       path: '/erp-client/events',       icon: '⚡' },
    { name: 'Trips',        path: '/erp-client/trips',        icon: '🚚' },
  ],
};

// Items that require specific module flags to appear.
// When a flag is absent from the modules response, we fall back to showing
// the item (safe default: existing workspaces have ordering=true).
const MODULE_GATE: Partial<Record<string, keyof ApiWorkspaceModules>> = {
  '/transport/orders': 'ordering',
  '/transport/trips':  'dispatch_api',
};
...
export default function Sidebar({ role }: { role: UserRole }) {
  ...
  const [modules, setModules] = useState<ApiWorkspaceModules | null>(null);

  useEffect(() => {
    api.workspace.get()
      .then(ws => setModules(ws.modules))
      .catch(() => { /* show all items on error — safe for Mode A workspaces */ });
  }, []);

  const allItems = baseNav[role] ?? [];

  // Filter based on module flags. While modules haven't loaded yet, show all
  // items — this avoids a flash of empty nav for Mode A (ordering=true) workspaces.
  const items = modules === null
    ? allItems
    : allItems.filter(item => {
        const flag = MODULE_GATE[item.path];
        if (!flag) return true;                   // no gate — always show
